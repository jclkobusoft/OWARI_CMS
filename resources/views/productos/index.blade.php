  ...
  {{ Html::script('cms/vendors/js/tables/buttons.html5.min.js') }}
    ...
    $('.scroll-horizontal').DataTable( {
        ...
        buttons: [
            {
                ...
                action: function ( e, dt, button, config ) {
                  window.location = '{{ route('productos.create') }}';
                }   
            },
            {
                text: '<i class="fa fa-file"></i>&nbsp;&nbsp;Acciones',
                className: 'btn mr-1 mb-1 btn-info btn-sm',
                action: function ( e, dt, button, config ) {
                  window.location = '{{ route('productos.excel') }}';
                }   
            }
        ...
        "scrollX": true,
	        ...
	        columns: [
                { data: 'marca_comercial', name: 'marca_comercial' },
                { data: 'codigo_nikko', name: 'codigo_nikko' },
                { data: 'descripcion_1', name: 'descripcion_1' },
                { data: 'especial', name: 'especial' },
                { data: 'precio_normal', name: 'precio_normal' },
                {
                  data: null,
                  title: 'ACCIONES',
                  orderable: false,
                  ...
                  render: (_, __, row) => {
                    const c = encodeURIComponent(row.id ? row.id : '');
                    const editar= `/admin/productos/editar/${c}`;
                    const borrar= `/admin/productos/borrar/${c}`; // si aplica
                    return `
                      <a href="${editar}" class="btn btn-sm btn-outline-secondary me-1">Editar</a>
                      <a href="${borrar}" class="btn btn-sm btn-outline-danger"
                        onclick="return confirm('¿Borrar ${row.codigo_nikko}?')">Borrar</a>
                    `;
                  }
                }
	        ]
...
@section('contenido')
	 <section id="horizontal">
              ...
              <div class="card">
                ...
                <div class="card-content collapse show">
                  <div class="card-body card-dashboard">
                    <p class="card-text">Agrega, edita y elimina informacion relacionada con los productos registrados en el sistema.</p>
         ...
         {{ Form::open(['route' => ['productos.destroy',':IDENTIFICADOR'],'method' => 'DELETE','id' => 'form-delete']) }}
    {{ form::close() }}
@endsection